...
**附註：**若要讓使用者產生 {{site.data.keyword.BluVirtServers_short}} 和 {{site.data.keyword.baremetal_short}} {{site.data.keyword.cloudaccesstrailshort}} 事件，該使用者必須要有在 {{site.data.keyword.cloud_notm}} 主控台中存取「基礎架構」資源的權限。如需相關資訊，請參閱[使用 {{site.data.keyword.cloudaccesstrailshort}} 來監視 {{site.data.keyword.BluVirtServers_short}} 和 {{site.data.keyword.baremetal_short}} 活動](/docs/services/cloud-activity-tracker/tutorials?topic=cloud-activity-tracker-vsi#vsi)。
...
| [{{site.data.keyword.baremetal_long}} ](/docs/bare-metal?topic=bare-metal-about-bm#about-bm) | {{site.data.keyword.baremetal_short}} 是單一承租戶的實體伺服器，可在對硬體資源進行低階存取的情況下，為您提供效能與控制能力。 | [{{site.data.keyword.baremetal_short}} 所產生的事件](/docs/bare-metal?topic=bare-metal-bm-at-events#bm-at-events) | 
...
| [佈建及管理連結至 Cloud Foundry 空間的型錄服務](/docs/overview?topic=overview-ui#catalogcreate)|您可以佈建服務實例、重新命名服務實例、變更服務實例的方案，以及移除服務實例。</br>這些事件是針對 CF 空間中所佈建的服務而產生。| [與型錄服務互動時所產生的事件](/docs/services/cloud-activity-tracker/services?topic=cloud-activity-tracker-cf#cf_catalog) |  
...
|服務|說明| {{site.data.keyword.cloudaccesstrailshort}} 事件 |
|-------------|-------------|-------------|
| [登入 {{site.data.keyword.cloud_notm}}](/docs/iam?topic=iam-iamoverview#iamoverview)|您可以使用密碼、API 金鑰、授權碼或通行碼來登入 {{site.data.keyword.cloud_notm}}。身為聯合使用者，您可以使用一次性密碼或 API 金鑰，從指令行介面 (CLI) 登入。| [使用者或應用程式登入 {{site.data.keyword.cloud_notm}} 時所產生的事件](/docs/services/cloud-activity-tracker/services?topic=cloud-activity-tracker-at_events_iam#at_events_iam_login) |
...
|服務|說明| {{site.data.keyword.cloudaccesstrailshort}} 事件 |
|-------------|-------------|-------------|
| [{{site.data.keyword.cos_full_notm}}](/docs/services/cloud-object-storage?topic=cloud-object-storage-about#about-ibm-cloud-object-storage)|您可以使用 {{site.data.keyword.cos_full_notm}} 將資料儲存在 {{site.data.keyword.cloud_notm}} 中。會在多個地理位置之間對資料進行加密及傳播，並透過 HTTP 使用 REST API 來存取資料。| [{{site.data.keyword.cos_full_notm}} 所產生的事件](/docs/services/cloud-object-storage?topic=cloud-object-storage-at-events) |  
...
|服務|說明| {{site.data.keyword.cloudaccesstrailshort}} 事件 |
|-------------|-------------|-------------|
| [Watson Studio](https://dataplatform.cloud.ibm.com/docs/content/wsj/getting-started/overview-ws.html) |Watson Studio 可提供環境及工具，以透過分工合作的方式處理資料來解決您的商業問題。您可以選擇分析及視覺化資料、清理及塑型資料、吸收串流資料，或建立、訓練及部署機器學習模型所需的工具。| [Watson Studio 所產生的事件](https://dataplatform.cloud.ibm.com/docs/content/wsj/admin/at-events.html#ws) |  
| [Watson Machine Learning](https://dataplatform.cloud.ibm.com/docs/content/wsj/analyze-data/ml-overview.html)|您可以使用 Watson Machine Learning 來建置更準確的分析模型，並使用您自己的資料來進行訓練，以便您能夠部署以用於應用程式中。| [Watson Machine Learning 所產生的事件](https://dataplatform.cloud.ibm.com/docs/content/wsj/admin/at-events.html#wml) | 
| [Watson Knowledge Catalog](https://dataplatform.cloud.ibm.com/docs/content/wsj/catalog/overview-wkc.html)|Watson Knowledge Catalog 可提供資料原則架構支援的安全企業型錄管理平台。型錄會將資料和知識與需要使用它的人連接起來。資料原則架構可確保資料存取符合您的商業規則。| [Watson Knowledge Catalog 所產生的事件](https://dataplatform.cloud.ibm.com/docs/content/wsj/admin/at-events.html#wkc) |  
